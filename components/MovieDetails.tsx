...
const MovieDetails: React.FC<MovieDetailsProps> = ({ movie, onClose, botUsername, channelLink }) => {
  ...
  const handleWatch = (code: string) => {
    ...
    if (window.Telegram?.WebApp) {
      // Premium popup with cancel option
      // @ts-ignore
      window.Telegram.WebApp.showPopup({
        title: '🎬 Watch Video',
        message: 'ভিডিওটি বটে পাঠানো হবে। বট চ্যাট খুলবে এবং আপনি ভিডিও পাবেন।',
        buttons: [
          { id: 'cancel', type: 'cancel', text: 'Cancel' },
          { id: 'watch', type: 'default', text: '▶️ Watch Now' }
        ]
      }, (buttonId: string) => {
        if (buttonId === 'watch') {
        ...
        // If cancel, do nothing - mini app stays open
      });
    } else {
  ...
  const handleDownload = (downloadCode?: string, downloadLink?: string, fallbackCode?: string) => {
    if (downloadLink) {
      ...
      if (window.Telegram?.WebApp) {
        ...
        window.Telegram.WebApp.showPopup({
          title: '⬇️ Download',
          message: 'ডাউনলোড লিঙ্ক বটে পাঠানো হবে। বট চ্যাট খুলবে এবং আপনি লিঙ্ক পাবেন।',
          buttons: [
            { id: 'cancel', type: 'cancel', text: 'Cancel' },
            { id: 'download', type: 'default', text: '⬇️ Get Link' }
          ]
        }, (buttonId: string) => {
